...
public class AngularGenericInsertFormComponentWriter implements ComponentWriter {
	...
	public void writeScript(Entity entity) throws Exception {
		...
		try (GeneratorOutputFile file = Utils.getOutputResource(Utils.getFrontendPagesPath() + kebabCase + "/" + kebabCase + "-insert-form", StringUtils.camelToKebabCase(entity.getName()) + "-insert-form.component.ts", false)) {
	...
	public void writeStyles(Entity entity) throws Exception {
		String kebabCase = StringUtils.camelToKebabCase(entity.getName());
		try (GeneratorOutputFile file = Utils.getOutputResource(Utils.getFrontendPagesPath() + kebabCase + "/" + kebabCase + "-insert-form", StringUtils.camelToKebabCase(entity.getName()) + "-insert-form.component.css", false)) {
			if (file.hasAlreadyExisted()) {
				return;
			}
			file.writeln(0, "");
	...
	public void writeHTML(Entity entity) throws Exception {
		String kebabCase = StringUtils.camelToKebabCase(entity.getName());
		try (GeneratorOutputFile file = Utils.getOutputResource(Utils.getFrontendPagesPath() + kebabCase + "/" + kebabCase + "-insert-form", StringUtils.camelToKebabCase(entity.getName()) + "-insert-form.component.html", false)) {
			if (file.hasAlreadyExisted()) {
				return;
			}
