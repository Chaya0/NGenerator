...
public class AngularEntityWriter {
	...
	public void create(Entity entity) throws Exception {
		...
		try (GeneratorOutputFile file = WriterUtils.getOutputResource(WriterUtils.getFrontendFeaturesEntitiesPath() + StringUtils.camelToKebabCase(entity.getName()),
				StringUtils.camelToKebabCase(entity.getName()) + ".ts", true)) {
			...
			for (Attribute attribute : entity.getAttributes()) {
				if (attribute.getType().equals(AttributeType.ENUM)) file.writeln(0, "import { " + StringUtils.uppercaseFirst(attribute.getEnumName()) + " } from '../enums.model';");
			...
			for (Attribute attribute : entity.getAttributes()) {
				if (attribute.getType().equals(AttributeType.ENUM)) {
					file.writeln(1, attribute.getEnumName() + "!: " + StringUtils.uppercaseFirst(attribute.getEnumName()) + ";");
				} else {
