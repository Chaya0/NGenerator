...
public class AngularGenericDeleteActionComponentWriter implements ComponentWriter {
	...
	public void writeScript(Entity entity) throws Exception {
		...
		try (GeneratorOutputFile file = WriterUtils.getOutputResource(WriterUtils.getFrontendPagesPath() + kebabCase + "/" + kebabCase + "-delete-action", StringUtils.camelToKebabCase(entity.getName()) + "-delete-action.component.ts", false)) {
	...
	public void writeStyles(Entity entity) throws Exception {
		String kebabCase = StringUtils.camelToKebabCase(entity.getName());
		try (GeneratorOutputFile file = WriterUtils.getOutputResource(WriterUtils.getFrontendPagesPath() + kebabCase + "/" + kebabCase + "-delete-action", StringUtils.camelToKebabCase(entity.getName()) + "-delete-action.component.css", false)) {
			if (file.hasAlreadyExisted()) {
				return;
			}
			file.writeln(0, "");
	...
	public void writeHTML(Entity entity) throws Exception {
		String kebabCase = StringUtils.camelToKebabCase(entity.getName());
		try (GeneratorOutputFile file = WriterUtils.getOutputResource(WriterUtils.getFrontendPagesPath() + kebabCase + "/" + kebabCase + "-delete-action", StringUtils.camelToKebabCase(entity.getName()) + "-delete-action.component.html", false)) {
			if (file.hasAlreadyExisted()) {
				return;
			}
			file.writeln(0, "");
