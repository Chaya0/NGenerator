...
public class AngularGenericUpdateFormComponentWriter implements ComponentWriter {
	...
	public void writeScript(Entity entity) throws Exception {
		...
		try (GeneratorOutputFile file = Utils.getOutputResource(Utils.getFrontendPagesPath() + kebabCase + "/" + kebabCase + "-update-form", StringUtils.camelToKebabCase(entity.getName()) + "-update-form.component.ts", false)) {
	...
	public void writeStyles(Entity entity) throws Exception {
		String kebabCase = StringUtils.camelToKebabCase(entity.getName());
		try (GeneratorOutputFile file = Utils.getOutputResource(Utils.getFrontendPagesPath() + kebabCase + "/" + kebabCase + "-update-form", StringUtils.camelToKebabCase(entity.getName()) + "-update-form.component.css", false)) {
			if (file.hasAlreadyExisted()) {
				return;
			}
			file.writeln(0, "");
	...
	public void writeHTML(Entity entity) throws Exception {
		String kebabCase = StringUtils.camelToKebabCase(entity.getName());
		try (GeneratorOutputFile file = Utils.getOutputResource(Utils.getFrontendPagesPath() + kebabCase + "/" + kebabCase + "-update-form", StringUtils.camelToKebabCase(entity.getName()) + "-update-form.component.html", false)) {
			if (file.hasAlreadyExisted()) {
				return;
			}
